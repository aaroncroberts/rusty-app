//! SQL Server connection form component
//!
//! Provides a form interface specifically for SQL Server connections with
...
/// SQL Server connection form data
#[derive(Debug, Clone, PartialEq)]
pub struct SQLServerConnectionFormData {
    pub name: String,
    pub host: String,
    pub port: String,
    pub database: String,
...
impl Default for SQLServerConnectionFormData {
    fn default() -> Self {
        Self {
            name: String::new(),
...
impl SQLServerConnectionFormData {
    /// Create a new SQL Server connection form with default values
    pub fn new() -> Self {
        Self::default()
    }
...
/// Messages for the SQL Server connection form
#[derive(Debug, Clone)]
pub enum SQLServerConnectionFormMessage {
    NameChanged(String),
    HostChanged(String),
    PortChanged(String),
...
/// SQL Server connection form component
#[derive(Debug, Clone)]
pub struct SQLServerConnectionForm {
    theme: ThemeColors,
}

impl SQLServerConnectionForm {
    /// Create a new SQL Server connection form with the given theme
    pub fn new(theme: ThemeColors) -> Self {
        Self { theme }
    }

    /// Render the SQL Server connection form
    pub fn view<'a, Message: 'a + Clone>(
        &'a self,
        data: &'a SQLServerConnectionFormData,
        testing_connection: bool,
        test_result: &'a Option<Result<(), String>>,
        on_message: impl Fn(SQLServerConnectionFormMessage) -> Message + 'a + Copy,
    ) -> Element<'a, Message> {
        ...
        let mut form_fields = column![
            ...
            self.form_field(
                "Connection Name",
                text_input("My SQL Server DB", &data.name)
                    .on_input(move |s| on_message(SQLServerConnectionFormMessage::NameChanged(s)))
                    .padding(8)
            ...
            self.form_field(
                ...
                text_input("localhost", &data.host)
                    .on_input(move |s| on_message(SQLServerConnectionFormMessage::HostChanged(s)))
                    .padding(8)
                    .style(|theme, status| self.input_style(theme, status))
            ),
            // Port
            self.form_field(
                "Port",
                text_input("1433", &data.port)
                    .on_input(move |s| on_message(SQLServerConnectionFormMessage::PortChanged(s)))
                    .padding(8)
                    .style(|theme, status| self.input_style(theme, status))
            ),
            // Database
        ...
        let test_button = if testing_connection {
            ...
            button(text("Test Connection").size(14))
                .padding([8, 16])
                .on_press(on_message(SQLServerConnectionFormMessage::TestConnection))
        ...
        let buttons = row![
            button(text("Cancel").size(14))
                .padding([8, 16])
                .on_press(on_message(SQLServerConnectionFormMessage::Cancel))
                .style(move |_theme, status| button::Style {
                    background: Some(theme.background_secondary.into()),
                    text_color: theme.text,
                    border: Border {
            ...
            button(text("Save Connection").size(14))
                .padding([8, 16])
                .on_press(on_message(SQLServerConnectionFormMessage::Save))
                .style(move |_theme, status| button::Style {
...
mod tests {
    ...
    #[test]
    fn test_sqlserver_connection_form_data_default() {
        let data = SQLServerConnectionFormData::default();
        assert_eq!(data.host, "localhost");
        assert_eq!(data.port, "1433");
        assert!(data.name.is_empty());
        assert!(data.database.is_empty());
    }

    #[test]
    fn test_sqlserver_connection_form_data_new() {
        let data = SQLServerConnectionFormData::new();
        assert_eq!(data.port, "1433");
    }

    #[test]
    fn test_validate_sqlserver_valid() {
        let data = SQLServerConnectionFormData {
            name: "Test".to_string(),
            host: "localhost".to_string(),
            port: "1433".to_string(),
    ...
    #[test]
    fn test_validate_sqlserver_missing_name() {
        let data = SQLServerConnectionFormData {
            name: "".to_string(),
            host: "localhost".to_string(),
            port: "1433".to_string(),
    ...
    #[test]
    fn test_sqlserver_connection_form_creation() {
        let theme = ThemeColors::dark();
        let form = SQLServerConnectionForm::new(theme);
